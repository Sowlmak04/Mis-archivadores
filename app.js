...
function normalizeData(candidate) {
  ...
  normalized.zones = Array.isArray(normalized.zones)
    ? normalized.zones.map(({ order, ...zone }) => zone)
    : [];
...
function sortedLocations() {
  return [...data.locations].sort((a, b) => a.name.localeCompare(b.name, "es", { sensitivity: "base", numeric: true }));
}
...
function zonesEditView(expandedLocations = new Set()) {
  screenTitle.textContent = "Editar zonas";
  setActiveTab("");
  app.innerHTML = `
    <section class="detail-header settings-header"><h2>Ubicaciones y zonas</h2><p>Crea la estructura que necesites. Toca una ubicación para desplegar o plegar sus zonas.</p></section>
    ...
    <section class="structure-editor">
      ${data.locations.length ? sortedLocations().map(loc => {
        ...
        return `<article class="structure-card ${expanded ? "is-expanded" : ""}" data-location-card="${loc.id}">
          <div class="structure-head">
            <button class="structure-toggle" type="button" data-toggle-location="${loc.id}" aria-expanded="${expanded}" aria-controls="location-body-${loc.id}">
              <span class="structure-chevron">${uiIcon("chevronDown")}</span>
              <span class="structure-title"><strong>${escapeHTML(loc.name)}</strong><small>${zones.length} zonas · ${total} contenedores</small></span>
            </button>
            <div class="mini-actions">
              <button class="icon-action" type="button" data-rename-location="${loc.id}" aria-label="Renombrar ${escapeHTML(loc.name)}" title="Renombrar">${uiIcon("edit")}</button>
              <button class="icon-action danger-text" type="button" data-delete-location="${loc.id}" aria-label="Eliminar ${escapeHTML(loc.name)}" title="Eliminar">${uiIcon("trash")}</button>
            </div>
          </div>
          <div class="structure-body" id="location-body-${loc.id}" ${expanded ? "" : "hidden"}>
            <div class="zone-editor-list ${zones.length ? "has-zones" : ""}">
              ${zones.length ? zones.map((zone, index) => {
                ...
                return `<div class="zone-editor-row" data-zone-row="${zone.id}"><span class="tree-branch" aria-hidden="true">${index === zones.length - 1 ? "└" : "├"}</span><div class="zone-editor-copy"><strong>${escapeHTML(zone.name)}</strong><small>${count} contenedores</small></div><div class="mini-actions"><button class="icon-action" type="button" data-rename-zone="${zone.id}" aria-label="Renombrar ${escapeHTML(zone.name)}" title="Renombrar">${uiIcon("edit")}</button><button class="icon-action danger-text" type="button" data-delete-zone="${zone.id}" aria-label="Eliminar ${escapeHTML(zone.name)}" title="Eliminar">${uiIcon("trash")}</button></div></div>`;
  ...
  app.querySelectorAll("[data-toggle-location]").forEach(btn => btn.addEventListener("click", () => {
    const id = btn.dataset.toggleLocation;
    expandedLocations.has(id) ? expandedLocations.delete(id) : expandedLocations.add(id);
    zonesEditView(expandedLocations);
  }));
  app.querySelector("#locationForm")?.addEventListener("submit", e => {
    ...
    data.locations.push({id:newId("loc"),name}); saveData(); zonesEditView(expandedLocations); showToast("Ubicación añadida");
  });
  app.querySelectorAll("[data-add-zone]").forEach(form => form.addEventListener("submit", e => {
    ...
    data.zones.push({id:newId("zone"),locationId,name}); saveData(); expandedLocations.add(locationId); zonesEditView(expandedLocations); showToast("Zona añadida");
  }));
  app.querySelectorAll("[data-rename-location]").forEach(btn => btn.addEventListener("click", e => {
    e.stopPropagation(); const loc=locationById(btn.dataset.renameLocation); const name=window.prompt("Nuevo nombre de la ubicación:",loc.name)?.trim(); if(!name)return;
    loc.name=name; saveData(); zonesEditView(expandedLocations); showToast("Ubicación actualizada");
  }));
  app.querySelectorAll("[data-rename-zone]").forEach(btn => btn.addEventListener("click", () => {
    ...
    zone.name=name; saveData(); zonesEditView(expandedLocations); showToast("Zona actualizada");
  }));
  app.querySelectorAll("[data-delete-zone]").forEach(btn => btn.addEventListener("click", () => {
    ...
    if(!window.confirm(msg))return; data.containers=data.containers.filter(c=>c.zoneId!==zone.id); data.zones=data.zones.filter(z=>z.id!==zone.id); saveData(); zonesEditView(expandedLocations); showToast("Zona eliminada");
  }));
  app.querySelectorAll("[data-delete-location]").forEach(btn => btn.addEventListener("click", e => {
    e.stopPropagation(); const loc=locationById(btn.dataset.deleteLocation); const count=data.containers.filter(c=>c.locationId===loc.id).length; const zcount=data.zones.filter(z=>z.locationId===loc.id).length;
    if(!window.confirm(`Eliminar “${loc.name}” eliminará también ${zcount} zonas y ${count} contenedores.\n\n¿Continuar?`))return;
    data.containers=data.containers.filter(c=>c.locationId!==loc.id); data.zones=data.zones.filter(z=>z.locationId!==loc.id); data.locations=data.locations.filter(l=>l.id!==loc.id); expandedLocations.delete(loc.id); saveData(); zonesEditView(expandedLocations); showToast("Ubicación eliminada");
  }));
}

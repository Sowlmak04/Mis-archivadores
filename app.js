      ...
      <section class="grid">
        ${data.locations.map(loc => {
          ...
          return `<button class="card" data-location="${loc.id}">
            <div class="card-icon">⌂</div><h3>${escapeHTML(loc.name)}</h3>
...
function zonesEditView() {
  ...
  app.innerHTML = `
    ...
    <section class="structure-editor">
      ${data.locations.length ? data.locations.map(loc => {
        const zones = data.zones.filter(z => z.locationId === loc.id);
        ...
        return `<article class="structure-card" data-location-card="${loc.id}">
          ...
          <div class="zone-editor-list">
            ${zones.length ? zones.map(zone => {
              const count=data.containers.filter(c=>c.zoneId===zone.id).length;
              return `<div class="zone-editor-row"><div><strong>${escapeHTML(zone.name)}</strong><small>${count} contenedores</small></div><div class="mini-actions"><button data-rename-zone="${zone.id}">Renombrar</button><button class="danger-text" data-delete-zone="${zone.id}">Eliminar</button></div></div>`;
            }).join("") : `<p class="structure-note">Sin zonas. Los contenedores se guardan directamente aquí.</p>`}
          </div>
          <form class="add-zone-form" data-add-zone="${loc.id}">
            <input maxlength="60" placeholder="Nueva zona" autocomplete="off" required ${direct ? "disabled" : ""}/>
            <button class="secondary-button" type="submit" ${direct ? "disabled" : ""}>+ Zona</button>
          </form>
          ${direct ? `<p class="helper">Esta ubicación tiene ${direct} contenedores directos. Para dividirla en zonas, elimina primero esos contenedores.</p>` : ""}
        </article>`;
  ...
  app.querySelectorAll("[data-add-zone]").forEach(form => form.addEventListener("submit", e => {
    ...
    data.zones.push({id:newId("zone"),locationId,name}); saveData(); zonesEditView(); showToast("Zona añadida");
  }));
...
function personalizationView() {
  ...
  setActiveTab("");
  app.innerHTML = `<section class="detail-header settings-header"><h2>Personalización</h2><p>Esta sección queda preparada para las próximas opciones de apariencia.</p></section><section class="placeholder-card"><strong>Próximamente</strong><p>Aquí podremos configurar tema, apariencia y otras preferencias sin mezclar esos cambios con la nueva estructura editable.</p></section>`;
}

...
const seedData = {
  ...
  zones: [
    { id: "salon-kallax", locationId: "salon", name: "Kallax" },
    { id: "salon-mueble-tv", locationId: "salon", name: "Mueble TV" },
    { id: "salon-mueble-pelis", locationId: "salon", name: "Mueble Pelis" },
    { id: "salon-aparador", locationId: "salon", name: "Aparador" },

    { id: "oficina-kallax", locationId: "oficina", name: "Kallax" },
    { id: "oficina-mueble-empotrado", locationId: "oficina", name: "Mueble empotrado" },
    { id: "oficina-armario-largo", locationId: "oficina", name: "Mueble largo" },

    { id: "habitacion-canape-cama", locationId: "habitacion", name: "Canapé cama" },
    { id: "habitacion-armario-superior", locationId: "habitacion", name: "Armario superior" },

    { id: "luna-kallax", locationId: "habitacion-luna", name: "Kallax" },
    { id: "luna-comoda-visi", locationId: "habitacion-luna", name: "Cómoda Visi" },
    { id: "luna-armario-cuna", locationId: "habitacion-luna", name: "Armario cuna" },

    { id: "terraza-habita-armario-empotrado", locationId: "terraza-habita", name: "Armario empotrado" },
    { id: "terraza-habita-armario-superior", locationId: "terraza-habita", name: "Armario superior" }
  ],
  containers: []
};
...
function normalizeToV07(source) {
  const loaded = clone(source);

  const childToParent = {
    "salon-kallax-cajon-izquierdo": "salon-kallax",
    "salon-kallax-cajon-central": "salon-kallax",
    "salon-kallax-cajon-derecho": "salon-kallax",
    "oficina-kallax-cajon-izquierdo": "oficina-kallax",
    "oficina-kallax-cajon-central": "oficina-kallax",
    "oficina-kallax-cajon-derecho": "oficina-kallax",
    "oficina-mueble-empotrado-inferior": "oficina-mueble-empotrado",
    "oficina-mueble-empotrado-superior": "oficina-mueble-empotrado",
    "luna-kallax-cajon-inferior": "luna-kallax",
    "luna-comoda-visi-cajon-inferior": "luna-comoda-visi",
  ...
  const locationOnlyZones = new Set([
    "terraza-salon-sillon-izquierdo",
    "terraza-salon-sillon-derecho"
  ]);

  loaded.locations = clone(seedData.locations);
  loaded.zones = clone(seedData.zones);

  loaded.containers = (loaded.containers || []).map(container => {
    const migrated = { ...container };

    if (childToParent[migrated.zoneId]) {
      migrated.zoneId = childToParent[migrated.zoneId];
    } else if (locationOnlyZones.has(migrated.zoneId) || migrated.locationId === "terraza-salon") {
      migrated.zoneId = null;
      migrated.locationId = "terraza-salon";
    }

    const zone = loaded.zones.find(item => item.id === migrated.zoneId);
    if (zone) migrated.locationId = zone.locationId;
...
function loadData() {
  try {
    ...
    const source = stored ? JSON.parse(stored) : clone(seedData);
    const loaded = normalizeToV07(source);
...
function validateImportedData(payload) {
  ...
  const normalized = normalizeToV07(candidate);
  const locationIds = new Set(normalized.locations.map(item => item?.id).filter(Boolean));
  const zoneIds = new Set(normalized.zones.map(item => item?.id).filter(Boolean));
  ...
  for (const container of normalized.containers) {
    const directInLocation = container.locationId === "terraza-salon" && !container.zoneId;
    const validZone = container.zoneId && zoneIds.has(container.zoneId);

    if (!container?.id || !container?.name || !locationIds.has(container.locationId) ||
        (!directInLocation && !validZone)) {
      ...
      ${data.locations.map(loc => {
        ...
        return `
          <button class="card" data-location="${loc.id}">
            ...
            <p>${count === 0 ? (data.zones.some(z => z.locationId === loc.id) ? "Sin contenedores" : "Sin zonas · 0 contenedores") : `${count} contenedores`}</p>
          </button>
        `;
      }).join("")}
    </section>
...
function locationView(locationId) {
  ...
  if (!zones.length) {
    app.innerHTML = `
      ...
      <div class="actions">
        <button class="primary-button" data-add-location-container>+ Añadir contenedor</button>
      </div>
    `;
...
function newLocationContainerView(locationId) {
  ...
  const zones = data.zones.filter(zone => zone.locationId === locationId);
  if (zones.length) return locationView(locationId);
